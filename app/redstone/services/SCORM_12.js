/*
 * SCORM 1.2 
 * boilerplate below is from ADL - code has been adapted to use with Angular, Require
 */
define(['../services'], function (services) {
  ...
  ****************************************************************************/
  ...
  services.factory('SCORM_12', function() {
    ...
    *******************************************************************************/
    function doLMSGetValue(name)
    {
      ...
      {
        err = new ErrorHandler(); // get why doLMSInitialize() returned false
    ...
    *******************************************************************************/
    function ErrorHandler()
    {
      var err = {'code':_NoError.code, 'string':_NoError.string, 'diagnostic':_NoError.diagnostic};
      var api = getAPIHandle();

      if (api === null)
      {
        message('Unable to locate the LMS\'s API Implementation.\nCannot determine LMS error code.');
        err.code = _GeneralException.code;
        err.string = _GeneralException.string;
        err.diagnostic = 'Unable to locate the LMS\'s API Implementation. Cannot determine LMS error code.';
        return err;
      }

      // check for errors caused by or from the LMS
      err.code = api.LMSGetLastError().toString();
      if (err.code !== _NoError.code)
      {
        // an error was encountered so display the error description
        err.string = api.LMSGetErrorString(err.code);
        err.diagnostic = api.LMSGetDiagnostic('');
      }
    ...
    *******************************************************************************/
    function findAPI(win)
    {
      ...
      while ((win.API === null) && (win.parent !== null || win.parent !== undefined) && (win.parent !== win))
      {
        findAPITries++;
    ...
    *******************************************************************************/
    function message(str)
    {
      if(debug)
      {
        output.log('SCORM 1.2 message: '+str);
      }
    }
    ...
    return {
      ...
      doLMSSetValue: function(name, value) {
        return doLMSSetValue(name, value);
      },
      doLMSCommit: function() {
        return doLMSCommit();
      },
      doLMSGetLastError: function() {
        return doLMSGetLastError();
      },
      doLMSGetErrorString: function(errorCode) {
        return doLMSGetErrorString(errorCode);
      },
      doLMSGetDiagnostic: function(errorCode) {
        return doLMSGetDiagnostic(errorCode);
      },
      doSetLocation: function (screenId) {
        return doLMSSetValue('cmi.core.lesson_location', screenId);
      },
      doGetLocation: function () {
        return doLMSGetValue('cmi.core.lesson_location');
      },
      doGetStudentId: function() {
        return doLMSGetValue('cmi.core.student_id');
      },
      doGetStudentName: function() {
        return doLMSGetValue('cmi.core.student_name');
      },
      doGetEntry: function() {
        return doLMSGetValue('cmi.core.entry');
      },
      doSetLessonStatus: function(status) {
        return doLMSSetValue('cmi.core.lesson_status', status);
      },
      doExit: function(mode) {
        return doLMSSetValue('cmi.core.exit', mode);
      }
    };
  });
});
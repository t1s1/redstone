/*
 * Experience API
 * Wrapper from https://github.com/adlnet/xAPIWrapper
 */
define(['../services'], function (services) {
  'use strict';

  services.factory('xAPI', function() {
    ...
    ADL.verbs = {
      ...
      "exited" : {"id" : "http://adlnet.gov/expapi/verbs/exited","display" : {"en-US" : "exited"}
      },
      ...
      "preferred" : {"id" : "http://adlnet.gov/expapi/verbs/preferred","display" : {"en-US" : "preferred"}
      },
      ...
      "shared" : {"id" : "http://adlnet.gov/expapi/verbs/shared","display" : {"en-US" : "shared"}
      },
    ...
    XAPIWrapper = function(config, verifyxapiversion) {
      ...
      if (verifyxapiversion && testConfig.call(this)) {
        window.ADL.XHR_request(this.lrs, this.lrs.endpoint+"about", "GET", null, null, function(r) {
          if(r.status == 200) {
            try {
    ...
    XAPIWrapper.prototype.sendStatement = function(stmt, callback) {
      if (this.testConfig()) {
        ...
        var resp = ADL.XHR_request(this.lrs, this.lrs.endpoint+"statements", "POST", JSON.stringify(stmt), this.lrs.auth, callback, {"id":id});
        
        if (!callback) {
          return {"xhr":resp, "id" :id};
        }
      }
    };
    ...
    XAPIWrapper.prototype.sendStatements = function(stmtArray, callback) {
      if (this.testConfig()) {
        ...
        var resp = ADL.XHR_request(this.lrs,this.lrs.endpoint+"statements", "POST", JSON.stringify(stmtArray), this.lrs.auth, callback);
      ...
      function requestComplete() {
        if(!finished){
          ...
          if (xhr.status === undefined || (xhr.status >= 200 && xhr.status < 400) || notFoundOk) {
            ...
            try {
                alert("There was a problem communicating with the Learning Record Store. ( " + xhr.status + " | " + xhr.response+ " )" + xhr.url);
